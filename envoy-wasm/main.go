...
func (ctx *kMamizFilterContext) OnHttpStreamDone() {
	if ctx.reqOutput != "" {
		ctx.dispatchRequest(ctx.reqOutput)
		proxywasm.LogInfo(ctx.reqOutput)
		ctx.reqOutput = ""
	}
	if ctx.resOutput != "" {
		ctx.dispatchRequest(ctx.resOutput)
		proxywasm.LogInfo(ctx.resOutput)
		ctx.resOutput = ""
	}
}

func (ctx *kMamizFilterContext) dispatchRequest(log string) {
	headers := [][2]string{
		{":method", "POST"},
		{":path", os.Getenv("DATA_PROCESSOR_PATH")},
		{":authority", os.Getenv("DATA_PROCESSOR")},
		{"accept", "*/*"},
		{"Content-Type", "text/plain"},
	}
	if _, err := proxywasm.DispatchHttpCall(os.Getenv("DATA_PROCESSOR"), headers, []byte(log), nil, 5000, nil); err != nil {
...
func (ctx *kMamizFilterContext) createLogInfo(direction int, headers *[][2]string) (string, bool, bool) {
	...
	isJson := headerMap["content-type"] == "application/json"
	isTarget := headerMap["x-b3-traceid"] != "NO_ID"
	if isTarget {
		output = fmt.Sprintf(
			"%s\t%s\t%s\t%s",
